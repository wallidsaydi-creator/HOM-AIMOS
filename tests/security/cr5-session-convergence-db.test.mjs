...
test('CR5 session SAVE proves typed Housekeeper actions, security order, continuity, and restart parity', async () => {
  ...
  for (const terminalRow of terminals) {
    ...
    assert.equal(trace.stages[3].status, 'DISABLED');
    assert.deepEqual(trace.stages[3].evidence, {
    ...
    assert.equal(String(terminal.parent_event_id), String(canary.id));
  }
  ...
  assert.equal(quarantinedTerminal.stages[3].status, 'DISABLED');
});
