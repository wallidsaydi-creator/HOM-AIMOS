...
test('clean release live-fire binds an absent canonical database without creating it', () => {
  const runner = read('scripts/test/run-isolated-security.mjs');
  assert.match(runner, /if \(error\?\.code !== '3D000'\) throw error/);
  assert.match(runner, /database_exists: false,[\s\S]*memories: 0,[\s\S]*sha256\(JSON\.stringify\(\[\]\)\)/);
  assert.match(runner, /database_exists: true,[\s\S]*memories: result\.rowCount/);
  assert.match(runner, /JSON\.stringify\(canonicalBefore\) !== JSON\.stringify\(canonicalAfter\)/);
  assert.doesNotMatch(runner, /CREATE DATABASE aimos|createdb[^\n]*aimos/);
});
